...
void update (float *uu, float *vv, float *pp, int nx, int ny);
...
int main ()
{
	...
	{
	update (&u[0][0], &v[0][0], &p[0][0], NROWS, NCOLS);
	}
...
void update (float *uu, float *vv, float *pp, int nx, int ny)
{
	...
	float u[nx-1][ny], 		v[nx][ny-1], 		p[nx][ny];
	float u_new[nx-1][ny], 	v_new[nx][ny-1],		p_new[nx][ny];
